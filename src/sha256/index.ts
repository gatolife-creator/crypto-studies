...
export function padding(msg: number[]) {
  let len = msg.length;
...
function SHR(x: number, n: number) {
  return x >>> n;
}
...
// メッセージをN個のメッセージブロックに分割する
// 各メッセージブロックは64byteとなる
export function compute(msg: number[]) {
  let N = msg.length / 64; // 64はブロックサイズ
  let W: number[] = [];
  let H: number[] = []; // 最終的にハッシュとして返される値
  ...
  for (let i = 1; i <= N; i++) {
    for (let t = 0; t < 64; t++) {
      if (t < 16) {
        let p = (i - 1) * 64 + t * 4;
        W[t] =
          (msg[p] << 24) + (msg[p + 1] << 16) + (msg[p + 2] << 8) + msg[p + 3];
      } else {
        W[t] =
...
export function hash2str(hash: number[]) {
